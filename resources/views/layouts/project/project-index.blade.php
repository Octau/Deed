            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="pt-2 relative mx-auto text-gray-600 ml-auto flex justify-end mb-2 items-center">
                        <a href="{{route('project.add')}}" :active="request()->routeIs('project.add')" class="text-gray-600 fill-current mx-5">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200" id="table_content">
                                            @foreach ($projects as $project)
                                                <tr scope="row">
                                                    ...
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" align="center"> {{ $project->status }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ $project->created_at }}</td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{{ $project->updated_at }}</td>
                                                    {{-- <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center"><a href="{{ route('supplier.edit', ['supplier_id' => $project->id])}}" ><img src="{{ asset('img/edit.svg') }}" alt="Edit"></a></td>
                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><a href="{{ route('supplier.delete', ['supplier_id' => $project->id])}}">DELETE</a></td> --}}
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
